import asyncio  # It is used when we have to run multiple api calls that do not depend on each other
import logging
import os

import nest_asyncio  # nest_asyncio modifies/patches the existing asyncio behavior so that an event loop can be nested inside another running event loop.
...
async def validate_calendar_request(
    ...
    CalendarValidation
):  # async means it has to wait until that service sends back a response.
    """Check if the input is a valid calendar request"""
    completion = await client.beta.chat.completions.parse(  # Await means wait for this asynchronous operation to finish, and then give me its result
        model=model,
        messages=[
            {
                "role": "system",
