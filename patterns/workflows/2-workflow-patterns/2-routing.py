from typing import (
    Optional,
    Literal,
)  # literal meaning only specific values are allowed which are predefined
...
class CalenderRequestType(BaseModel):
    """Router LLM call: Determine the type of calender request"""

    request_type: Literal["new_event", "modify_event", "other"] = Field(
        description="Type of calender request being made"
    )
...
class Change(BaseModel):
    ...
    name: str = Field(description="Field to change")
...
class ModifyEventDetails(BaseModel):
    ...
    partcipants_to_remove: list[str] = Field(description="Participants to remove")
...
def process_calendar_request(user_input: str) -> Optional[CalendarResponse]:
    ...
    """ This is routing is done using if and else"""
